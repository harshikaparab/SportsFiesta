    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // console.log('events response: ' + JSON.stringify(eventsResponse.data));

                if (eventsResponse.data.length > 0) {
                    const latestEvent = eventsResponse.data[eventsResponse.data.length - 1]; // Assuming events are sorted by date
                    // console.log('latestevent: ' + JSON.stringify(eventsResponse.data[0]));
                ...
                >
                    {latestEventLeaderboard.length > 0 && events.length > 0 ? (
                        <BeautifulBarGraph 
                            ...
                            eventName={events[events.length-1].title} 
                        />
                    ) : (
